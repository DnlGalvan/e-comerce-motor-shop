...
export const StyledHeader = styled.header`
	...
	border-bottom: 0.125rem solid var(--grey6);
	z-index: 20;
	.link {
		font-weight: 600;
		font-size: 1rem;
...
export const MenuAnimation = styled.span<SidebarProps>`
	...
	background-color: ${(props) =>
		props.isSideBarVisible ? "transparent" : "black"};
...
export const DropDown = styled.div<DropdownProps>`
	...
	pointer-events: ${(props: DropdownProps) =>
		props.dropdown === 0 ? `none` : `all`};
...
export const Sidebar = styled.div<SidebarProps>`
	...
	@media (max-width: 900px) {
		...
		pointer-events: ${(props: SidebarProps) =>
			props.isSideBarVisible ? `all` : `none`};
		border-top: 0.2rem solid var(--grey6);
